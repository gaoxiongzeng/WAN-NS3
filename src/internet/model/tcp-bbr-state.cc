...
void BbrStateMachine::changeState(BbrState *new_state) {
  ...
  if (m_state)
    NS_LOG_LOGIC(this << "  Old: " << m_state->GetName()
...
BbrState::BbrState(TcpBbr *owner) { m_owner = owner; }

BbrState::BbrState() { m_owner = NULL; }
...
// Get name of object.
std::string BbrState::GetName() const { return "BbrState"; }
...
BbrStartupState::BbrStartupState(TcpBbr *owner)
    : BbrState(owner), m_full_bw(0), m_full_bw_count(0) {
  NS_LOG_FUNCTION(this);
}

BbrStartupState::BbrStartupState()
    : BbrState(), m_full_bw(0), m_full_bw_count(0) {
  NS_LOG_FUNCTION(this);
}
...
// Get name of object.
std::string BbrStartupState::GetName() const { return "BbrStartupState"; }
...
void BbrStartupState::execute() {
  ...
  if (new_bw > m_full_bw * bbr::STARTUP_THRESHOLD) {
    NS_LOG_LOGIC(this << "  Still growing. old_bw: " << m_full_bw
                      << "  new_bw: " << new_bw);
    m_full_bw = new_bw;
    m_full_bw_count = 0;
    return;
  ...
  m_full_bw_count++;
  NS_LOG_LOGIC(this << "  Growth stalled. old_bw: " << m_full_bw << "  new_bw: "
                    << new_bw << "  full-bw-count: " << m_full_bw_count);
...
BbrDrainState::BbrDrainState(TcpBbr *owner)
    : BbrState(owner), m_inflight_limit(0), m_round_count(0) {
  NS_LOG_FUNCTION(this);
}

BbrDrainState::BbrDrainState()
    : BbrState(), m_inflight_limit(0), m_round_count(0) {
  NS_LOG_FUNCTION(this);
}
...
// Get name of object.
std::string BbrDrainState::GetName() const { return "BbrDrainState"; }

// Get state type.
bbr::bbr_state BbrDrainState::getType(void) const { return bbr::DRAIN_STATE; }
...
void BbrDrainState::execute() {
  ...
  NS_LOG_LOGIC(this << " " << GetName() << "  round: " << m_round_count
                    << "  bytes_in_flight: "
                    << m_owner->m_bytes_in_flight_adjusted
                    << "  inflight_limit: " << m_inflight_limit);

  // See if should exit DRAIN state.
  // Do when byte-in-flight are under limit or 3 rounds
  // have passed (2.89-1)/(1-1/2.89) ~ 3), whichever is first.
  m_round_count++;
  uint32_t force_round = 1;
  if (m_owner->m_bytes_in_flight_adjusted < m_inflight_limit ||
      m_round_count >= force_round) {
...
BbrProbeBWState::BbrProbeBWState() : BbrState() { NS_LOG_FUNCTION(this); }
...
// Get name of object.
std::string BbrProbeBWState::GetName() const { return "BbrProbeBWState"; }
...
void BbrProbeBWState::execute() {
  ...
  // Set gain rate: [stdy, stdy, high (random), low (after high), stdy, stdy,
  // stdy, stdy]
  if (m_gain_cycle == m_high_cycle)
  ...
  NS_LOG_LOGIC(this << " " << GetName()
                    << " DATA pacing-gain: " << m_owner->m_pacing_gain);
}
...
BbrProbeRTTState::BbrProbeRTTState() : BbrState() { NS_LOG_FUNCTION(this); }
...
// Get name of object.
std::string BbrProbeRTTState::GetName() const { return "BbrProbeRTTState"; }
...
void BbrProbeRTTState::enter() {
  ...
  NS_LOG_LOGIC(this << " " << GetName() << " In PROBE_RTT until: "
                    << m_probe_rtt_time.GetSeconds());
}
